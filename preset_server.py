...
def cleanup_auto_generated(agents_dir: Path) -> None:
    """自動生成されたエイリアスディレクトリを削除する。"""
    for d in _find_auto_generated_dirs(agents_dir):
        try:
            shutil.rmtree(d)
            logger.info("PromoteAgents: cleaned up %s", d.name)
        except Exception as e:
            logger.warning("PromoteAgents: failed to clean up %s: %s", d.name, e)


def generate_alias_dirs(agents_dir: Path) -> list[Path]:
    """各エージェントの preset.yaml の promote_agents を読み、エイリアスディレクトリを生成する。
    ...
    for agent_dir in sorted(agents_dir.iterdir()):
        ...
        try:
            ...
            logger.warning("PromoteAgents: failed to read %s: %s", preset_path, e)
            continue
        ...
        # promote_agents は dict 形式: { agent_name: { module?: str, initial_state?: dict, ... } }
        for agent_name, entry in promote_agents.items():
            ...
            # 既存の非自動生成ディレクトリとの衝突を回避
            if alias_dir.exists() and not (alias_dir / _AUTO_GENERATED_MARKER).exists():
                logger.warning(
                    "PromoteAgents: skipping %s — directory already exists (not auto-generated)",
                    alias_name,
                )
                continue
            ...
            (alias_dir / _AUTO_GENERATED_MARKER).write_text(
                f"Auto-generated by preset_server.py from {parent_name}\n"
            )

            # .gitignore: ディレクトリ内を全て無視（ディレクトリごと git 管理外になる）
            (alias_dir / ".gitignore").write_text("*\n", encoding="utf-8")

            # agent_path: {parent}.{module}.{agent_name}
            agent_path = f"{parent_name}.{module}.{agent_name}"

            # preset.yaml を生成
            alias_preset: dict = {"agent_path": agent_path}
            # initial_state: エントリ固有 > 親の initial_state
            state = alias_state or data.get("initial_state")
            if state:
                alias_preset["initial_state"] = state
            initial_query = entry.get("initial_query") or data.get("initial_query")
            if initial_query:
                alias_preset["initial_query"] = initial_query
            ...
            logger.info("PromoteAgents: generated alias %s -> %s", alias_name, agent_path)
...
def cli_preset_web(
    ...
    # --- promote_agents: エイリアスの自動生成 ---
    # まず前回の残りをクリーンアップ（kill -9 対策）
    cleanup_auto_generated(agents_dir_path)
    # preset.yaml の promote_agents からエイリアスを生成
    generated = generate_alias_dirs(agents_dir_path)
    if generated:
        names = [d.name for d in generated]
        click.secho(f"PromoteAgents: generated aliases: {', '.join(names)}", fg="cyan")
    # 終了時にクリーンアップ
