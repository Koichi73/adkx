...
def generate_alias_dirs(agents_dir: Path) -> list[Path]:
    ...
    for agent_dir in sorted(agents_dir.iterdir()):
        ...
        for entry in promote_agents:
            # entry は dict（attr 必須、name はオプション）またはただの文字列
            if isinstance(entry, str):
                ...
                attr = entry.get("attr")
                if not attr:
                    logger.warning("PromoteAgents: promote_agents entry missing 'attr' in %s", preset_path)
                    continue
                alias_name = entry.get("name")
                alias_state = entry.get("initial_state")

            if not alias_name:
                attr_short = attr.rsplit(".", 1)[-1]
                alias_name = f"{parent_name}_{attr_short}"
            ...
            # agent_path を . 区切りのモジュールパスで生成（attr はフルパス例: "agent.greet_agent"）
            agent_path = f"{parent_name}.{attr}"
